...
class ConditionalLayerNorm(nn.Module):
    def __init__(self,
        ...
        self.conditon_type_embeddings = nn.Embedding(1, 128)
    ...
    def forward(self, inputs, conds):
        assert conds is not None, 'Conditional tensor need to input when use conditional layer norm'
        conds = self.conditon_type_embeddings(conds)
        cond = torch.unsqueeze(conds, 1)  # (b, 1, h*2)
...
class RoFormerEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, token_type_ids=None, inputs_embeds=None, conds=None):
        ...
        # embeddings = self.LayerNorm(embeddings, conds)
